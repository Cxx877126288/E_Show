        ...
        <c:forEach items="${goods}" var="g">
            <div class="goods left" id="${g.goodsid}">
                ...
                <div class="mybutton left">
                    <input id="car" type="button" value="加入购物车" style=" font-size: 9px;font-weight: bold;" onclick="carClick(this)" title="加入购物车">
                    <input id="information" type="button" value="查看详情" style=" font-size: 9px;font-weight: bold;" onclick="carClick(this)" title="查看详情">
                </div>
            </div>
        </c:forEach>
        <div class="left clpage">
            第${nowpage}页/共${count}页
                <a href="/show?secondname=${sename}&pagenumber=1">首页</a>
            <%--href="/show?secondname=${sename}&pagenumber=${nowpage - 1}"--%>
            <a <c:if test="${nowpage > 1}">href="/show?secondname=${sename}&pagenumber=${nowpage - 1}"</c:if>
               <c:if test="${nowpage <= 1}">href="/show?secondname=${sename}&pagenumber=${nowpage}"</c:if>
            >上一页</a>
                <c:forEach var="i" begin="1" end="${count}">
            <a href="/show?secondname=${sename}&pagenumber=${i}">${i}</a>
        </c:forEach>
                <a
                   <c:if test="${nowpage < count}">href="/show?secondname=${sename}&pagenumber=${nowpage + 1}"</c:if>
                   <c:if test="${nowpage >= count}">href="/show?secondname=${sename}&pagenumber=${nowpage}"</c:if>
                >下一页</a>
            <a href="/show?secondname=${sename}&pagenumber=${count}">尾页</a>
        </div>
    </c:if>
</div>
</body>
